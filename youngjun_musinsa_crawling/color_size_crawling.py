...
# Helper function to split a list into n nearly equal parts
...
def visit_websites(description_list):
    ...
    # Split the description_list into sublists
    sub_lists = split_list(description_list, 4)

    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(visit_website, sub_list) for sub_list in sub_lists]
        for future in concurrent.futures.as_completed(futures):
            try:
                sizes, colors = future.result()
                size_options.extend(sizes)
                color_options.extend(colors)
            except Exception as e:
                print(f"Exception occurred while visiting websites: {e}")
                size_options.extend([[] for _ in range(len(sub_lists[0]))])
                color_options.extend([[] for _ in range(len(sub_lists[0]))])
